...
public class RegistrationServiceImpl implements RegistrationService {
    ...
    public String verifyToken(TokenConfirmationRequest tokenConfirmationRequest) {
        ...
        voterService.enableVoter(tokenConfirmationRequest.getEmail());
        return "User has been Confirmed!";
    }

    @Override
    public String resendToken(ResendTokenRequest resendTokenRequest) {
        Voter foundVoter = voterService.findByEmailAddressIgnoreCase(
                        resendTokenRequest.getEmailAddress()).
                orElseThrow(() ->
                        new IllegalStateException(
                                "This email does not exist"));
        if (foundVoter.getIsVerified().equals(true))
            throw new RegistrationException("Already verified");
        String token = generateToken(foundVoter);
        emailSender.send(
                resendTokenRequest.getEmailAddress(),
                buildEmail(foundVoter.getFirstName(), token));
